...
public class ClampAutonomousBRed extends OpMode{
    ...
    public void loop() {
        angles = imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
        if(!trip1){
            rampUpSide(one*2.95, 0.0, 0.5, 0.5);
            trip1 = tripLoopSideways(one*2.95);
        }
        else if(!trip2){
            rampUp(-one*3.85, 0.0, 0.5, 0.3);
            trip2 = tripLoop(-one*3.85);
        }
        else if (!clampDown){
            moveClampsDown();
        }
        else if (!trip3){
            rampUp(one*0.09, 0.0, 0.5,0.5);
            trip3 = tripLoop(one*0.09);
        }
        else if (!trip4){
            ...
            rampUp(-one*1.0, -90.0, 0.5, 0.5);
            trip5 = tripLoop(-one*1.0);
        }
        else if (!clampUp){
            moveClampsUp();
        }
        else if (!trip6){
            rampUp(one*4.9, -90.0, 0.5, 0.75);
            trip6 = tripLoop(one*4.9);
        }
        /*else if (!trip7){
