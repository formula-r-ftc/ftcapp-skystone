...
public class StoneClampAutonomousLRed extends OpMode{
    ...
    private boolean grabSkystone(){
        //TODO: Write code to grab Skystone
        return true;
    }
    private boolean dropSkystone(){
        //TODO: Write code to drop Skystone
        return true;
    }
    ...
    public void loop() {
        ...
        if (!trip1) {
            ...
            trip1 = tripLoopSideways(-one*2.1);
        }
        /*else if (!trip2) {
            rampUp(-one*moveBack1, 0.0, 0.5, 0.2);// asof; hofh eih f
            trip2 = tripLoop(-one*moveBack1);
        }*/
        else if (!grabSkystone) {
            grabSkystone = grabSkystone();
        }
        else if (!trip3) {
            rampUpSide(one*1.7, 0.0, 0.5, 0.5);
            trip3 = tripLoopSideways(one*1.7);
        }
        else if (!trip4) {
            ...
            rampUpTurn(92.0, 1.0, 0.5);
            trip5 = tripLoopTurn(92.0);
            telemetry.addData("HeadingAccuracy", 90-getHeading());
            telemetry.update();
            if(trip5){
                t1.reset();
            }
        ...
        /*else if (!trip9) {
            rampUpSide(one*2.0, 0.0, 0.5, 0.5);
            trip9 = tripLoopSideways(one*2.0);

            telemetry.addData("Encoder PositionRFRF", one*2.0 - RFMotor.getCurrentPosition());
            telemetry.update();
        ...
        else if (!grabSkystone2) {
            grabSkystone2 = grabSkystone();
        }
        else if (!trip11) {
        ...
        else if (!dropSkystone2) {
            dropSkystone2 = dropSkystone();
        }
        else if (!trip14) {
